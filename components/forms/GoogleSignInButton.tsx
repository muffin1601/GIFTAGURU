 ...
 *
 * Its own form, separate from the email/password form beside it, so pressing
 * it never submits (or validates) the credential fields.
 */
export default function GoogleSignInButton({ next, label }: { next?: string; label: string }) {
  const [state, formAction, pending] = useActionState<AuthState, FormData>(signInWithGoogleAction, {});
...
/** Google's official four-colour mark. Inline so it needs no network fetch. */
function GoogleMark() {
  return (
